...
class Squeezeformer(nn.Module):
  ...
  def __init__(self,
    ...
    reduce_idxes = FLAGS.reduce_idxes or [FLAGS.time_reduce_idx, FLAGS.time_reduce_idx]
    assert len(reduce_idxes) == 2
    ic(reduce_idxes)
    ## in this version share reduce module..
    reduce_module = TimeReductionModule(dim, dim, kernel_size=FLAGS.time_kernel_size, stride=FLAGS.time_stride)
    conv_kernel_size_ = conv_kernel_size
    for i in range(depth):    
      if i in reduce_idxes:
        self.layers.append(reduce_module)
      if i < reduce_idxes[0]:
        dim_head = 32
        heads = 4
        conv_kernel_size_ = conv_kernel_size
        ...
        if reduce_idxes[1] > reduce_idxes[0]:
          dim_head = 32
          heads = 16
          conv_kernel_size_ = conv_kernel_size // 4
