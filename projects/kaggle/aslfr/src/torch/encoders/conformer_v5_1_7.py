...
class Attention(nn.Module):
  def __init__(self, dim, dim_head=64, 
                heads=8, dropout=0., max_pos_emb=512, rope=False):
    super().__init__()
...
class Conformer(nn.Module):
  ...
  def __init__(self,
    ...
    for i in range(depth):    
      rope = False if i < FLAGS.nonrope_layers else True    
    ...
    if FLAGS.time_reduce:
      if FLAGS.time_reduce_idx < depth - 1:
        self.layers.insert(FLAGS.time_reduce_idx, TimeReductionModule(dim, dim, kernel_size=5, stride=2))
