...
#     #     exclude_from_weight_decay=['layer_norm', 'bias'])
    ...
    optimizer_type = names.get(optimizer_type.lower(), optimizer_type)
    ...
    if 'use_ema' in args:
        kwargs_['use_ema'] = FLAGS.opt_ema
    if 'ema_momentum' in args:
        kwargs_['ema_momentum'] = FLAGS.opt_ema_momentum
    if optimizer_type.lower() == 'adamw':
      optimizer = Opt(learning_rate=lr_schedule, 
                      ...
                      epsilon=epsilon,
                      **kwargs_)
    elif optimizer_type.lower() == 'rectifiedadam':
      optimizer = Opt(learning_rate=lr_schedule, 
                      sma_threshold=FLAGS.opt_sma_threshold,
                      **kwargs_)
    else:
      optimizer = Opt(learning_rate=lr_schedule, **kwargs_)

  logging.info('ori optimizer:', optimizer, 'with kwargs', kwargs_)
...
#     optimizer = tfa.optimizers.MovingAverage(optimizer, average_decay=FLAGS.opt_ema_momentum)
  ...
  if FLAGS.opt_lookahead:
    optimizer = tfa.optimizers.Lookahead(optimizer, 
                                         sync_period=FLAGS.opt_sync_period, 
                                        #  slow_step_size=FLAGS.opt_slow_step_size
                                         )
  return optimizer
