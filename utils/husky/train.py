...
def train(model, 
  ...
  with strategy.scope():
    ...
    gezi.set('num_steps_per_epoch', steps_per_epoch_)
    ...
    if FLAGS.pretrained_dir:
      ...
      if FLAGS.pretrain_online:
        pretrained_dir = pretrained_dir.replace('/offline', '/online')
    else:
      pretrained_dir = None
    if pretrained_dir and not FLAGS.pretrain_restart:
      if not os.path.exists(f'{FLAGS.model_dir}/model_step.txt'):
        gezi.system(f'cp {pretrained_dir}/model_step.txt {FLAGS.model_dir}')
    ...
    if FLAGS.reset_global_step:
      ...
      try:
        ...
        # TODO 可能是在compile还是什么时候会把_train_counter继续归0 这里assign实际并不起作用
        model._train_counter.assign(model_step)
        model._step.assign(model_step)
        # ic(model.step, model._train_counter)
        if not iterations:
    ...
    #   model = AWP(model.input, model.output, delta=0.2, eps=0., start_step=int(FLAGS.adv_start_epoch * steps_per_epoch))
    ...
    if FLAGS.keras_tensorboard:
      ...
      # start = 10
      # end = FLAGS.profile_interval_steps or 100
      ...
      tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=log_dir, profile_batch=2, write_graph=FLAGS.write_graph)
    ...
    if FLAGS.last_models:
      callbacks.append(SaveLastModelsCallback(FLAGS.num_epochs - FLAGS.last_models))
    elif FLAGS.last_model:
      callbacks.append(SaveLastModelCallback(FLAGS.num_epochs - FLAGS.last_model))
    elif FLAGS.inter_models:
      callbacks.append(SaveInterModelsCallback([int(x) for x in FLAGS.inter_models], FLAGS.exit_epoch or FLAGS.epochs))
    if FLAGS.exit_epoch:
      callbacks.append(ExitCallback(FLAGS.exit_epoch))
      # if initial_epoch < FLAGS.exit_epoch:
      #   epochs = FLAGS.exit_epoch
    ...
    # ic(callbacks)
    logging.debug('callbacks', callbacks)
    ...
    if FLAGS.cv_valid_only or FLAGS.cv_test_only:
      ...
      if not dry_run:
        ...
        if FLAGS.custom_loop:
          ...
          dataset_ = iter(dataset)
          for epoch in tqdm(range(epochs)):
            for _ in tqdm(range(steps_per_epoch), total=steps_per_epoch):
              x, y = next(dataset_)
              strategy.run(train_step, args=(x, y))
        else:
          ic(initial_epoch, epochs)
