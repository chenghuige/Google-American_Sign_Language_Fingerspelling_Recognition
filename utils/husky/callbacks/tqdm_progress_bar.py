  ...
  def _update_progbar(self, logs):
    ...
    if self.steps_so_far <= self.total_steps:
      ...
      if self.show_epoch_progress and time_diff >= self.update_interval:
        # TODO move it to callbacks.py ? current here ok? 4.5 g only show 0.05 ?
        # self.logs['mem'] = tf.config.experimental.get_memory_info('GPU:0')['current'] / 1024 / 1024 / 1024
        try:
          self.logs['mem'] = tf.config.experimental.get_memory_info('GPU:0')['peak'] / (1024 ** 3)
        except Exception:
          pass
