...
class CheckpointCallback(Callback):
  ...
  def load(self, ckpt_dir):
    ...
    try:
      ...
      logging.warning(e)
      l2 = 0.
    model_size = '%.2f' % (total_params * 4 / (1024 * 1024))
    logger.info(f'before loading, total params: {total_params}, l2:{l2:.6f}, model_size: {model_size}M')
    ...
    if FLAGS.bakup_model:
      gezi.system(f'mkdir -p {FLAGS.model_dir}/bak; cp {FLAGS.model_dir}/model* {FLAGS.model_dir}/bak/')
    return loaded
...
class UtilsCallback(Callback):
  ...
  def work(self):
    ...
    logging.info('Model total training parameters is:', total_params, 'with initial l2:', l2, f'model size: {model_size}M')
    self.l2 = l2
...
def copy_model(src_dir, dest_dir, files=[]):
  ...
  files += map(os.path.basename, glob.glob(f'{src_dir}/model.ckpt*'))
...
class SaveInterModelsCallback(Callback):
...
class SaveLastModelsCallback(Callback):
...
class SaveIntervalModelsCallback(Callback):
