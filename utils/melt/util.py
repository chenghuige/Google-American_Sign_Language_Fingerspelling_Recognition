...
if tf.__version__ > '2':
  # TODO FIXME tensorflow.python.keras depreciated and break in tf2.13(from tensorflow.python.keras.engine import data_adapter not work anymore)
  # https://github.com/huggingface/transformers/issues/23352#issuecomment-1559699535
...
class Model(keras.Model):
  ...
  def __init__(self, model=None, accum_step=None, interval_steps=0, **kwargs):
    ...
    if FLAGS.awp_train:
      # https://github.com/hoyso48/tf-utils/blob/main/tf_utils/learners.py
      # TODO 也许 class AWP(mt.Model) then model = AWP(model.input, model.output, delta=CFG.awp_lambda, eps=0., start_step=awp_step) is better?
      # 当前这么写Model类越来越大 好处是对外更加统一 不关心细节e
      self.delta = 0.2 if FLAGS.adv_lr is None else FLAGS.adv_lr
      self.eps = 0. if FLAGS.adv_eps is None else FLAGS.adv_eps
      ic(FLAGS.adv_start_epoch, self.delta, self.eps)
      # if self.step >= adv_start_step:
      #   return self.awp_train_step(data)
      self.adv_start_step = None
  ...
  def monitor_emb(self, emb, name='emb', zero_ratio=False, training=True):
    if training:
      # TODO 这些应该不起作用。。 如果图模式 只保留编译时期的python值...
  ...
  def train_step(self, data):
    ...
    # self._train_counter.assign(self.step)
      
    # self._step will restore after restart while _train_counter always start from 0 TODO
    if FLAGS.log_level < 10:
      tf.print('train_step', self._step, self._train_counter)
      
    if FLAGS.awp_train:
      if self.adv_start_step is None:
        steps_per_epoch = gezi.get('num_steps_per_epoch')
        adv_start_epoch = FLAGS.adv_start_epoch or 0
        self.adv_start_step = int(adv_start_epoch * steps_per_epoch)
        ic(self.adv_start_step)
        # tf.print('adv_start_step', self.adv_start_step)
      # TODO check diff with torch implementation, here delta is same as lr ther
      # TODO checked work ok 但是这样会减速 哪怕adv start epoch 16 那么前面正常不使用awp的15 epoch也会变慢很多2.8 -> 2it/s
      if self.adv_start_step > 0:
        # TODO not work for multiple gpus 
        ...
        return tf.cond(self._step < self.adv_start_step, lambda:self.basic_train_step(data), lambda:self.awp_train_step(data))
      else:
        return self.awp_train_step(data)
  ...
  def basic_train_step(self, data):
    if FLAGS.debug:
      tf.print('basic_train_step', self._step, self._train_counter)
    # self.step += 1
    ...
    # # TODO .. self.step 总是2 self._training_counter才在变化...
    # if self.step == 1 or not self.interval_steps or self.step % self.interval_steps == 0:
  ...
  def awp_train_step(self, data):
    if FLAGS.debug:
      tf.print('awp_train_step', self._step, self._train_counter)
    ## TODO self.step +1 not work as in tf.function only +1 once...
    # self.step += 1
    self._step.assign_add(1)
  ...
  def loss_wrapper(self, loss_fn, inputs=None, model=None):
    ...
    def loss_fn_(y_true, y_pred):
      ...
      if 'inputs' in args:
        kwargs['inputs'] = inputs or self.input_ or self.inputs
      if 'model' in args:
        kwargs['model'] = model or self
        
      for k, v in kwargs.items():
        assert v is not None, f'{k} is None, be sure to set self.input_=inputs in call()'
...
def get_lr(optimizer):
  try:
    lr = optimizer.lr(optimizer.iterations)
  except Exception:
...
def model_name_from_args(ignores=[], sep='.'):
  ...
  ignores = set(ignores)
  for arg in args:
    if arg.startswith('--ignore_cmds='):
      ignores.update(arg.split('=')[1].split(','))
  ...
  for arg in args:
    ...
    if ok:
      ...
      if FLAGS.mn:
        ...
        FLAGS.mn = arg.replace('flagfile', '').replace('flags/', '').replace(
