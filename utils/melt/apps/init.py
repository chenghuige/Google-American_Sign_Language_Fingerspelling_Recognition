...
import fcntl
from multiprocessing import log_to_stderr, cpu_count
import gezi
import melt
...
def _set_strategy():
  if FLAGS.num_gpus > 1 and not FLAGS.torch:
    ...
    ic(FLAGS.distribute_strategy, FLAGS.num_gpus)
...
def init(graph=None):
  ...
  progress_bar = FLAGS.pdl_progressbar if dist.local_rank() == 0 else False
  pdl.initialize(nb_workers=FLAGS.pdl_workers, progress_bar=progress_bar)
  tqdm.pandas()
  ...
  if FLAGS.model_name_suffix and FLAGS.mode != 'async_valid':
    ...
    if FLAGS.model_name_suffix in ['.debug', '.test']:
      FLAGS.wandb = False
  ...
  if (not FLAGS.clear_first):
    ...
    if FLAGS.restart:
      gezi.system(f'rm -rf {FLAGS.model_dir}/done.txt')
      
    if os.path.exists(f'{FLAGS.model_dir}/done.txt'):
      ic('Find done.txt')
      gezi.set('done', True)
      # if (FLAGS.mode == 'train' or FLAGS.mode is None) and (not FLAGS.folds_metrics) and (not FLAGS.kaggle_method):
      if FLAGS.mode == 'train' or FLAGS.mode is None:
        exit(0)      
  ...
  if FLAGS.retrain:
    FLAGS.pretrain = FLAGS.retrain
    FLAGS.pretrain_restart = False

  if FLAGS.restore_configs is None:
    if os.path.exists(f'{FLAGS.model_dir}/flags.pkl'):
      FLAGS.restore_configs = True
    else:
      FLAGS.restore_configs = False

  ## 默认restore configs 如果不需要的话 --restore_configs=0明确指出 或者删除model_dir 或者model_dir/flags.pkl      
  # if FLAGS.work_mode != 'train':
  #   FLAGS.restore_configs = False
  if FLAGS.restore_configs:
    pretrained = None
    if FLAGS.pretrained and not (os.path.exists(f'{FLAGS.model_dir}/flags.pkl')):
    ...
    ignore_names = [
            ...
            'batch_size', 'bs', 'eval_batch_size', 'eval_bs', 'fp16', 'float16', 'fold', 'folds', 'num_folds', 'online', 'input',
            'train_input', 'valid_input', 'test_input', 'train_exclude',
            'valid_exclude', 'train_only', 'max_metrics_show', 'gpu_metrics', 'show_keys',
            'steps', 'num_steps',
            ...
            'save_interval_steps', 'sie',
            'exit_epoch', 'torch_compile', 'tc',
        ]
    if (not FLAGS.torch) or FLAGS.restore_skip_epochs:
      # 这里因为torch 是可以重启dataset完整复现的采用相同seed skip前面的步骤 保证重启之后继续训练能完整恢复 当然代价是空loop dataset也有时间消耗 有些时候比较久
      # 可以手动设置skip 同时注意手动设置 --seed 保证 dataset不会重新跑之前的epoch的一样的aug数据
      # tf暂时无法做到一样的重启空loop,都是从dataset开始iter 为了避免dataset的epoch aug数据和重启之前一样 这里重启seed不加载 确保不一样的dataset loop
      # 当然也可以手动设置不一样的seed --seed=
      ignore_names += ['seed']
    ignore_names += gezi.get_commandline_flags() + FLAGS.restore_config_ignores
    restore_dir = pretrained or FLAGS.log_dir or FLAGS.model_dir
    if os.path.exists(f'{restore_dir}/flags.pkl'):
      logger.info(f'restore configs from {restore_dir}/flags.pkl with ingores {gezi.get_commandline_flags()}')
  ...
  if FLAGS.num_eval_steps:
    ...
    FLAGS.num_eval_per_epoch = int(FLAGS.num_eval_steps / FLAGS.num_epochs)
  ...
  # -------------------------setup gpus
  # TODO gpus 0,1,2,3 specific_gpus 4,5,6,7 tf problem?
  gpus_global = gpus
  # --change to local gpu index
  specific_gpus = gezi.get_specific_gpus()
  # ic(gpus, specific_gpus)
  if specific_gpus:
  ...
  FLAGS.valid_interval_epochs = FLAGS.valid_interval_epochs or FLAGS.ep
  ...
  replica_eval_batch_size = FLAGS.replica_eval_batch_size
  ...
  if num_threads: 
    ...
    if os.path.exists('/root/autodl-tmp'):
      ...
      num_threads = int(cpu_count() / gezi.get_total_num_gpus())
    torch.set_num_threads(num_threads)
  else:
    if os.path.exists('/root/autodl-tmp'):
      num_threads = int(cpu_count() / 8 * num_gpus)
    else:
      num_threads = int(cpu_count() / gezi.get_total_num_gpus() * num_gpus)
    torch.set_num_threads(num_threads or 16)
  ...
  if FLAGS.num_workers is None:
    if distributed:
      ...
      FLAGS.num_workers = num_gpus * 4

  ic(global_batch_size, acc_steps, replica_batch_size, 
     eval_batch_size, replica_eval_batch_size, 
     num_gpus, gpus, CUDA_VISIABLE_DEVICES, 
     num_threads, FLAGS.num_workers, 
     work_mode, distributed, seed)
  ...
  if FLAGS.local_rank > 0:
    FLAGS.wandb = False  
    FLAGS.folds_metrics = False
  
  if FLAGS.debug_loop:
    FLAGS.wandb = False
    FLAGS.folds_metrics = False
  
  if any(FLAGS.mn.endswith(x) for x in ['.debug', '.test', '.debug1', '.debug2', '.test1', '.test2']):
    FLAGS.wandb = False
    FLAGS.folds_metrics = False
    FLAGS.torch_compile = False
    # tf.data.experimental.enable_debug_mode() 
    # tf.config.run_functions_eagerly(True)
    # if FLAGS.torch_compile is None:
    #   FLAGS.torch_compile = False
      
  # TODO current default strategy for torch compile
  if FLAGS.torch_compile is None:
    if FLAGS.distributed:
      FLAGS.torch_compile = False
    else:
  ...
  # ------------wandb
  if FLAGS.wandb and (not gezi.get('wandb_run')):
    try:
  ...
  # TODO wandb 当前 不支持异步
  # TODO RuntimeError: Cannot re-initialize CUDA in forked subprocess. To use CUDA with multiprocessing, you must use the 'spawn' start method
  # 当前async eval不能被torch支持
  if FLAGS.wandb:
  ...
  if FLAGS.local_rank == 0:
    ...
    if FLAGS.work_mode == 'train' or (not os.path.exists(f"{FLAGS.log_dir}/flags.pkl")):
      try:
