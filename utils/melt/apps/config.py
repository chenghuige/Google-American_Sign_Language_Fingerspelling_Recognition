...
flags.DEFINE_bool('copy_to_device', False, 'https://cloud.tencent.com/developer/ask/sof/115589415')
...
flags.DEFINE_string('retrain', None, '')
flags.DEFINE_bool('pretrain_online', False, '')
flags.DEFINE_bool('pretrain_restart', True, '')
flags.DEFINE_string('pretrain_epoch', None, '')
flags.DEFINE_string('pretrain_subdir', None, '')
flags.DEFINE_bool('restore_pretrain_configs', False, '')
...
flags.DEFINE_bool('bakup_model', False, '')
...
flags.DEFINE_bool('restore_configs', None, '''Notice if None in melt.init will set to True if found flags.pkl in model_dir, why? 
                                              For if resume training we may want to restore seeds, if you need to change some config put it to -- and notice also for alias,
                                              or you could --cf if you do not need to resume''')
flags.DEFINE_alias('rc', 'restore_configs')
flags.DEFINE_list('restore_config_ignores', [], '')
flags.DEFINE_bool('restore_skip_epochs', True, '')
...
flags.DEFINE_integer('save_interval_epochs2', None, '')
flags.DEFINE_alias('sie2', 'save_interval_epochs2')
flags.DEFINE_integer('num_save_per_epoch', None, '')
flags.DEFINE_alias('nspe', 'num_save_per_epoch')
...
flags.DEFINE_bool('restore_pretrain_optimizer', True, '')
flags.DEFINE_bool('restore_pretrain_scheduler', True, '')
flags.DEFINE_bool('del_inter_model', False, '')
flags.DEFINE_alias('del', 'del_inter_model')
flags.DEFINE_bool('del_inter_events', False, '')

flags.DEFINE_bool('clear_callbacks', False, '')
flags.DEFINE_integer('exit_epoch', None, '')

#--------show
flags.DEFINE_integer('interval_steps', 100, '')
flags.DEFINE_alias('is', 'interval_steps')
flags.DEFINE_integer('valid_interval_steps', 100,   """for training suggest 100 if you want to see valid loss, and other valid metrics if set during training, 
...
flags.DEFINE_bool('gpu_metrics', False, '')
flags.DEFINE_integer('eval_interval_steps', None, '')
flags.DEFINE_alias('eis', 'eval_interval_steps')
flags.DEFINE_float('train_interval_epochs', 1, '')
flags.DEFINE_float('valid_interval_epochs', None, 'if 0 will be the same as num_epochs so only valid at the end, if > num_epochs will be num_epochs, if -1 will disable valid interval epochs')
flags.DEFINE_alias('eval_interval_epochs', 'valid_interval_epochs')
flags.DEFINE_alias('vie', 'valid_interval_epochs')
flags.DEFINE_alias('eie', 'valid_interval_epochs')
flags.DEFINE_integer('num_eval_per_epoch', None, '')
flags.DEFINE_alias('nepe', 'num_eval_per_epoch')
...
flags.DEFINE_bool('eval_train', False, '')
...
flags.DEFINE_float('opt_ema_momentum', 0.99, '')
flags.DEFINE_bool('opt_swa', False, '')
flags.DEFINE_bool('opt_lookahead', False, '')
flags.DEFINE_integer('opt_sync_period', 5, '')
flags.DEFINE_integer('opt_sma_threshold', 4, '')
flags.DEFINE_bool('opt_correct_bias', False, '')
flags.DEFINE_bool('opt_fused', False, '')
flags.DEFINE_bool('opt_ori', False, '')
flags.DEFINE_bool('fused_layernorm', False, '')
...
flags.DEFINE_bool('restart', False, '')
...
flags.DEFINE_integer('init_epoch', 0, 'set init_epoch to < 0 like -1 will load init_epoch from pretrain model when FLAGS.pretrain_restart=True')
...
flags.DEFINE_float('clip_gradients', 0, """follow im2text as 5.0 default, 
...
flags.DEFINE_integer('min_free_gpu_mem', None, 'by M, if 0 or < 0 means need all gpu mem exclusive gpu mode')
flags.DEFINE_integer('min_free_gpu_mem_valid', None, 'by M, if 0 or < 0 means need all gpu mem exclusive gpu mode')
...
flags.DEFINE_integer('fold', 0, 'by default 0 fold, if -1 means FLAGS.online=True, using all folds')
...
flags.DEFINE_bool('fp16', None, 'tpu可以优先fp16模式速度更快 但是很奇怪比float32更占存储空间更容易OOM 所以如果OOM优先先尝试fp16=False 如果还是OOM再batch_size减半')
flags.DEFINE_alias('float16', 'fp16')
flags.DEFINE_bool('bfloat16', False, '')
...
flags.DEFINE_bool('use_shard', False, '默认use shard就是走dataset shard,如果走eval callback和test callback自行处理设置False')
...
flags.DEFINE_bool('debug_inputs', False, '')
...
flags.DEFINE_bool('cache_after_map', False, 'https://www.tensorflow.org/guide/data_performance')
...
flags.DEFINE_bool('disable_valid_dataset', True, 'by defualt change to disable valid dataset? as eval is more important and valid dataset might hurt speed')
flags.DEFINE_alias('dvd', 'disable_valid_dataset')
flags.DEFINE_bool('disable_eval_dataset', False, '')
flags.DEFINE_alias('ded', 'disable_eval_dataset')
...
flags.DEFINE_bool('torch_compile', None, '')
flags.DEFINE_alias('tc', 'torch_compile')
flags.DEFINE_string('compile_mode', 'default', '')

flags.DEFINE_integer('num_threads', 0, """threads for reading input tfrecords,
                                           setting to 1 may be faster but less randomness
                                           or for torch.set_num_threads
                                        """)
flags.DEFINE_integer('num_dataset_threads', 0, '')
...
flags.DEFINE_bool('pb_loss_only', False, 'for tqdm progressbar only show loss realted metrics')
flags.DEFINE_bool('rich_tqdm', False, '')
flags.DEFINE_alias('rtqdm', 'rich_tqdm')
flags.DEFINE_string('tqdm_colour', 'green', 'None as white, cyan, blue')
flags.DEFINE_string('tqdm_colour2', 'blue', 'None as white, cyan, blue')
flags.DEFINE_integer('tqdm_nrows', None, '')
flags.DEFINE_bool('evshow_loss_only', True, '')
flags.DEFINE_bool('leave_epoch_progress', True, '')
flags.DEFINE_bool('leave_overall_progress', True, '')
flags.DEFINE_bool('eval_leave', True, '')
flags.DEFINE_bool('test_leave', True, '')
flags.DEFINE_bool('epoch_leave', True, '')
...
flags.DEFINE_bool('update_each_epoch', True, 'deal with None in melt.init, by default None means True when not run on notebook otherwise False')
...
flags.DEFINE_bool('enable_xla', False, 'seems somtimes enable xla will slow down need to check')
flags.DEFINE_string(
    "tpu_name", None,
...
flags.DEFINE_bool('async_eval', False, 'only cpu evaluation without prediction(already done and write to csv)')
...
flags.DEFINE_integer('pymp_workers', None, '')

flags.DEFINE_integer('pdl_workers', 16, '')
flags.DEFINE_bool('pdl_progressbar', True, '')
...
flags.DEFINE_bool('eval_distributed', False, '')
...
flags.DEFINE_string('dataset_device', 'cpu', 'maybe set to cpu or None or empty or gpu')
...
flags.DEFINE_bool('transform_batch', False, '')
flags.DEFINE_alias('batch_transform', 'transform_batch')
...
flags.DEFINE_list('ignore_cmds', [], '')

flags.DEFINE_integer('workers', 20, '')
flags.DEFINE_integer('num_workers', None, 'mainly for torch dataset')
...
flags.DEFINE_string('rdrop_key', 'pred', '')
...
flags.DEFINE_float('adv_start_epoch', None, '') 
flags.DEFINE_float('adv_eps', None, '')
flags.DEFINE_float('adv_lr', None, '')

flags.DEFINE_float('latedrop_start_epoch', None, '')
flags.DEFINE_float('lateaug_start_epoch', None, '')
