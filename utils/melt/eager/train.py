...
import lele
from lele import update_scalars
try:
...
def torch_(x):
  ...
  # if  tf.__version__ < '2':
  # for tf only.. eager to numpy then to torch
  x = x.numpy()
  ...
  if FLAGS.torch_only:
    for key in x:
      if isinstance(x[key], dict):
        ...
        try:
          if type(x[key][0]) not in [np.str_, str, np.bytes_, bytes, object]:
            if not torch.is_tensor(x[key]):
              x[key] = torch.from_numpy(x[key])
            x[key] = x[key].to(device, non_blocking=True)
        except Exception as e:
          ic(e, key, x[key], x[key].dtype, type(x[key][0]))
          if 'id' in x:
            ic(x['id'])
    if y is None:
      return x
...
def show_results(results, step, global_step, num_examples):
  if results:
    ...
    if not FLAGS.show_keys:
      ...
      for key in FLAGS.show_keys:
        if key in results:
          results_[key] = results[key]
          
    # results_['Metrics/val_time'] = FLAGS.valid_time
    results_['val_time'] = FLAGS.valid_time
    ...
    for key in scalars:
      ...
      if key in ['loss', 'val_loss']:
        ...
        if key == 'loss':
          results['train_loss'] = scalars[key]
      if FLAGS.show_keys and key not in FLAGS.show_keys:
        continue
      if key.startswith('val_metric/'):
        results_[key.replace('val_metric/', '')] = scalars[key]
    ...
    presults = gezi.dict_rename(results_, 'Metrics/', '')
    if FLAGS.show_keys:
      presults = {k: presults[k] for k in FLAGS.show_keys}
    gezi.pprint_dict(presults, print_fn=logging.info, 
                    ...
                    desc=f'[{FLAGS.model_name}] fold:{FLAGS.fold}/{FLAGS.folds} ' + 'epoch:%.2f/%d step:%d eval_step:%d examples:%d' % (gezi.epoch(), FLAGS.num_epochs, global_step, step, num_examples),
                    format='%.4f')
    
    results['epoch'] = gezi.epoch() + FLAGS.init_epoch
    ...
    results['Metrics/epoch'] = gezi.epoch() + FLAGS.init_epoch
    # ic(results_, results, scalars)
    if FLAGS.wandb:
      try:
        wandb.log(results)
      except Exception:
...
def eval(eval_fn, labels, predicts, other, step, global_step=None, ofile=None, show=True, do_eval=True, 
  ...
  if show:
    show_results(results, step, global_step, num_examples)
...
def evaluate(model, dataset, eval_fn, model_path=None, 
  ...
  with torch.no_grad():
    ...
    for step, item in ProgressBar(enumerate(dataset), total=num_steps, desc=desc, ascii=False, leave=leave, colour=FLAGS.tqdm_colour, disable=(rank!=0)):
      ...
      if not FLAGS.amp_infer:
        ...
        dtype = None if not FLAGS.bfloat16 else torch.bfloat16
      ...
      if FLAGS.torch:
        # like for bfloat16 TypeError: Got unsupported ScalarType BFloat16
        predicts = predicts.float().detach().cpu().numpy()
      ...
      if FLAGS.torch:
        ...
        for key in other:
          other[key] = other[key].float().detach().cpu().numpy()
...
def inference(model, dataset, model_path, 
  ...
  with torch.no_grad():
    if FLAGS.rich_tqdm:
      ...
      from gezi import tqdm as ProgressBar
    # ProgressBar = rtqdm
    for step, item in ProgressBar(enumerate(dataset), total=num_steps, desc=desc, ascii=False, leave=True, colour=FLAGS.tqdm_colour, disable=(rank!=0)):
      if isinstance(item, (list, tuple)):
        x, _ = item
      else:
        x = item
      ...
      if not FLAGS.amp_infer:
        ...
        dtype = torch.float16 if not FLAGS.bfloat16 else torch.bfloat16
        with torch.cuda.amp.autocast(dtype=dtype):
          if not FLAGS.torch and  'training' in inspect.getfullargspec(model.call).args:
            predicts = model(x, training=False)
          else:
...
def load_torch_model(path, model=None, optimizer=None, scheduler=None,
  ...
  if optimizer is not None:
    if 'optimizer' in checkpoint and (not (FLAGS.pretrain and (FLAGS.pretrain_restart or (not FLAGS.restore_pretrain_optimizer)))):
      optimizer.load_state_dict(checkpoint['optimizer'])
      logger.info('optimizer loaded')
  
  if scheduler is not None:
    if 'scheduler' in checkpoint and (not (FLAGS.pretrain and (FLAGS.pretrain_restart or (not FLAGS.restore_pretrain_scheduler)))):
       scheduler.load_state_dict(checkpoint['scheduler'])
       logger.info('scheduler loaded')

  checkpoint['init_epoch'] = checkpoint.get('init_epoch', 0)
  checkpoint['epoch'] = checkpoint.get('epoch', 0 if FLAGS.pretrain else FLAGS.num_epochs)
  checkpoint['step'] = checkpoint.get('step', 0)
  ic(checkpoint['init_epoch'], checkpoint['epoch'], checkpoint['step'])
  ...
  if FLAGS.pretrain and FLAGS.pretrain_restart:
    if FLAGS.init_epoch < 0:
      FLAGS.init_epoch = int(checkpoint['epoch'] + checkpoint['init_epoch'])
  ...
  if FLAGS.round == 0:
    if optimizer is None or isinstance(optimizer, str):
      ...
      if optimizer_name.lower() == 'adabelief':
        ...
        if FLAGS.opt_ori:
          # notice this is faster 1.78 to 1.8+ it/s then transformers.adamw with foreach=True, but seems performance is worse TODO
          ...
          try:
            ...
            logging.warning('apex not found, still use torch.optim.AdamW')
        ...
        from lele.training.optimizers import Lion as Optimizer
      elif optimizer_name.lower() == 'lookahead_adam':
        from lele.training.optimizers import LookaheadAdam as Optimizer
      elif optimizer_name.lower() == 'lookahead_radam':
        from lele.training.optimizers import LookaheadRAdam as Optimizer
      else:
      ...
      logging.info(f'Optimizer {Optimizer} with args {kwargs} lr:{FLAGS.learning_rate}, lrs:{FLAGS.learning_rates}')
      optimizer = Optimizer(params, **kwargs)
      ic(Optimizer, optimizer, scheduler)
...
def train(model, 
  ...
  if FLAGS.disable_valid_dataset:
    valid_dataset = None
  ...
  if FLAGS.torch:
    ...
    if FLAGS.last_models:
      callbacks.append(lele.SaveLastModelsCallback(FLAGS.num_epochs - FLAGS.last_models))
    elif FLAGS.last_model:
      callbacks.append(lele.SaveLastModelCallback(FLAGS.num_epochs - FLAGS.last_model))
    elif FLAGS.inter_models:
      callbacks.append(lele.SaveInterModelsCallback([int(x) for x in FLAGS.inter_models], FLAGS.exit_epoch or FLAGS.epochs))
    if FLAGS.exit_epoch:
      callbacks.append(lele.ExitCallback(FLAGS.exit_epoch))
  if FLAGS.clear_callbacks:
    callbacks = []
  ic(callbacks, len(callbacks))
  ...
  if dataset is None and (not FLAGS.mode in ['valid', 'test']):
    ...
    if FLAGS.mode not in ['valid', 'test']:
      ...
      if FLAGS.torch_only:
        ic(train_dataset, len(train_dataset.dataset), len(train_dataset))
        num_examples = FLAGS.num_train or len(train_dataset.dataset)
      else:
        ic(train_dataset, len(train_dataset))
        num_examples = FLAGS.num_train or len(train_dataset) * FLAGS.batch_size
    else:
      num_examples = 0
      train_dataset = None
  ...
  if eval_dataset is not None:
    ...
    if FLAGS.torch_only:
      ...
      num_valid_examples = FLAGS.num_full_valid or FLAGS.num_valid or len(eval_dataset) * FLAGS.eval_batch_size
    ...
    if valid_inputs and Dataset is not None:
      ...
      if not FLAGS.disable_valid_dataset:
  ...
  if num_examples:
    try:
      num_steps_per_epoch_ = len(train_dataset)
    except Exception:
      num_steps_per_epoch_ = None
    ...
    # this is because some dataset may have less data then int(num_examples / batch_size)
    # also some dataset might also be used ad dataloader so len(dataset) is num_examples.. not num_steps_per_epoch
    if num_steps_per_epoch_ is not None:
      if num_steps_per_epoch_ < num_steps_per_epoch:
        num_steps_per_epoch = num_steps_per_epoch_
  else:
  ...
  if test_dataset is not None:
    ...
    if FLAGS.torch_only:
      ...
      num_test_examples = FLAGS.num_test or len(test_dataset) * FLAGS.eval_batch_size
  else:
  ...
  if not FLAGS.torch:
    ...
    start_epoch = int(global_step.numpy() / num_steps_per_epoch) if num_steps_per_epoch else 0
  else:
    ...
    if latest_checkpoint is not None:
      ...
      if FLAGS.pretrained:
        ...
        if FLAGS.pretrain_online:
          pretrained = pretrained.replace('/offline', '/online')
    ...
    if FLAGS.round == 0:
      ...
      if latest_checkpoint:
        ...
        checkpoint = load_torch_model(latest_path, model=model, 
                                      ...
                                      map_location='cpu', includes=FLAGS.restore_include.split(','),
        ...
        if not FLAGS.torch_restart:
          ...
          if FLAGS.mode not in ['valid', 'test']:
            step = checkpoint['epoch'] * num_steps_per_epoch if 'epoch' in checkpoint else 0
          if FLAGS.train_loop:
  ...
  if FLAGS.torch:
    ...
    if FLAGS.round == 0:
      ...
      if FLAGS.distributed:        
        ...
        if optimizer:
          lele.optimizer_to(optimizer, device)
        model = torch.nn.parallel.DistributedDataParallel(model,
                                                          device_ids=[rank],
                                                          output_device=rank,
                                                          find_unused_parameters=FLAGS.find_unused_parameters)
        dist.barrier()
        logger.info('DDP model to device done')
        ...
        if optimizer:
          lele.optimizer_to(optimizer, device)
        logger.info('DP model to device done')
      else:
        model = model.to(device)
        if optimizer:
          lele.optimizer_to(optimizer, device)
        logger.info('Single gpu model to device done')
  ...
  if will_valid:
    ...
    if eval_fn and not 'dataset' in args:
      ...
      if eval_fn is not None:
        ...
        melt.inc_eval_step()
        show_results(results, melt.get_eval_step(), gezi.global_step(), num_valid_examples)
  ...
  total_steps = int(num_steps_per_epoch * FLAGS.num_epochs)
  ...
  if FLAGS.torch:
    ...
    model_size_ = '%.2f' % model_size
    logger.info(f'model_size: {model_size_}M num_params: {int(num_params / 1e6)}M {num_params} num_trainable_params: {int(num_trainable_params / 1e6)}M {num_trainable_params}')
    m = {
      'Others/total_params': num_params / 1e6,
  ...
  if FLAGS.fold is None or FLAGS.fold < 0:  
    pbar = tqdm(total=total_steps, desc=f'[{FLAGS.mn}] Epochs:{num_epochs}', colour=FLAGS.tqdm_colour, nrows=FLAGS.tqdm_nrows, disable=(rank!=0))
  else:
    pbar = tqdm(total=total_steps, desc=f'[{FLAGS.mn}_{FLAGS.fold}] Epochs:{num_epochs}', colour=FLAGS.tqdm_colour, nrows=FLAGS.tqdm_nrows, disable=(rank!=0))
  ...
  if FLAGS.torch_compile:
    # default: 适合加速大模型，编译速度快且无需额外存储空间
    # reduce-overhead：适合加速小模型，需要额外存储空间
    # max-autotune：编译速度非常耗时，但提供最快的加速
    # TODO 目前实验只有default work?... 不过确实有加速 但DDP多卡模式下 貌似没有 反而更慢了 可能因为batch size太小的无效...
    m = {
      'default': 'default',
    ...
    ic(m[FLAGS.compile_mode])
    model = torch.compile(model, mode=m[FLAGS.compile_mode])
    if FLAGS.distributed:
      torch.distributed.barrier()
    
  if not FLAGS.torch_only:
    train_dataset_ = iter(train_dataset)
  ...
  for epoch in range(0, end_epoch):
    ...
    # if FLAGS.restore_skip_epochs or (global_step.numpy() % num_steps_per_epoch == 0):
    if FLAGS.restore_skip_epochs and epoch < int(start_epoch):
      pbar.update(num_steps_per_epoch)
      cur_step += num_steps_per_epoch
      continue
    ...
    try:
      if cur_step < global_step.numpy():
        train_dataset.dataset.skip_steps = global_step.numpy() - cur_step
    except Exception:
      pass
          
    # ic(cur_step, global_step.numpy(), train_dataset.dataset.skip)
    # if not using tf dataset
    if FLAGS.torch_only:
      # TODO 注意如果这里train_dataset本身是repeat模式 notebook使用会tqdm会标红 因为没有loop完就结束了 不过连续for loop似乎也是ok的 等于迭代的调用next 并不会造成重复访问
      pbar2 = ProgressBar(enumerate(train_dataset), total=num_steps_per_epoch, desc=f'Epoch:{epoch + 1}', leave=FLAGS.epoch_leave, colour=FLAGS.tqdm_colour2, nrows=FLAGS.tqdm_nrows, disable=(rank!=0))
    else:
      pbar2 = ProgressBar(enumerate(range(num_steps_per_epoch)), total=num_steps_per_epoch, desc=f'Epoch:{epoch + 1}', leave=FLAGS.epoch_leave, colour=FLAGS.tqdm_colour2, nrows=FLAGS.tqdm_nrows, disable=(rank!=0))
    for i, item in pbar2:
      ...
      if not FLAGS.torch_only:
        item = next(train_dataset_)
      # TO test dataset speed only
      # ic(i, item)
      # ic(i, cur_step, global_step.numpy(), epoch)
      if cur_step < global_step.numpy():
        cur_step += 1
        pbar.update(1)
        continue
      ...
      if transform_batch_fn:
        x = transform_batch_fn(x)  
      ...
      postfix = {
        ...
        'mem': gezi.format_float_str(mem, 1)
      }
      postfix.update({
      ...
      def loss_fn_(x, y):
        ...
        def valid_():
          if valid_fn and ((not model.training) or (global_step.numpy() % FLAGS.interval_steps == 0)):
            import lele
            kwargs = {}
            ...
            lele.update_scalars(valid_metrics, decay=FLAGS.metric_decay, training=model.training)
        ...
        if FLAGS.torch:
          ...
          if do_rdrop:
            ...
            loss += rdrop_loss
            # move it after loss_fn only for DP, because DP might change res['pred'] in loss_fn  TODO
            valid_()
            return loss
          loss = loss_fn(y_, y, **kwargs)
          valid_()
          return loss
        else:
      ...
      if not FLAGS.debug_loop:
        ...
        if not FLAGS.torch:
          ...
          # torch training TODO remove if else by using below
          # scaler = GradScaler(enabled=args.use_amp) # makes scaler calls no-ops if false
          # ...
          # with autocast(enabled=args.use_amp) # makes autocast no-op if false
          # 允许在混合精度和普通 FP32 训练之间切换，无需任何手动 if/else 语句。）
          if scaler is None:
            loss = loss_fn_(x, y)
            # worker.prefetch()
            ...
            if hasattr(callback, 'on_retrain_begin'):
              if callback.on_retrain_begin(melt.epoch()):
                if scaler is None:
                  ...
                  # TODO some times awp train + fp16 here will case loss=nan, for some projects not affect final results for some will get bad results
                  # currently if you see nan loss, just try to use fp32 
                  # 也和project相关比如asl cls任务就没问题 但是ctc loss 这里就会有问题
                  dtype = None if not FLAGS.bfloat16 else torch.bfloat16
                  with torch.cuda.amp.autocast(dtype=dtype):
                    loss = loss_fn_(x, y)
                    loss = loss / FLAGS.acc_steps
      ...
      assert not np.isnan(loss_), 'try fp32 --fp16=0 first,(1 known issue is some project get nan with fp16 + awp training) if still nan, check your model and data'

      update_scalars({'loss': loss_}, decay=FLAGS.loss_decay)
        ...
        if rank == 0:
          if global_step.numpy() % FLAGS.valid_interval_steps == 0:
            ...
            m = {
              ...
              'History/lr': learning_rate.numpy() if hasattr(learning_rate, 'numpy') else learning_rate,
              'Perf/hours_per_epoch': hours_per_epoch,
              'Perf/steps_per_second': steps_per_second,
              'Perf/insts_per_second': instances_per_second,
              'Perf/mem': mem,
              'History/step': step,
              'History/epoch': melt.epoch() + FLAGS.init_epoch,
              'Perf/step': step,
              'epoch': melt.epoch() + FLAGS.init_epoch,
              'step': step
            }
            m.update(
              gezi.dict_prefix(gezi.get('scalars', {}), 'History/')
            )
            if 'History/loss' in m:
              m['History/train_loss'] = m['History/loss']
            if wandb_run:
      ...
      is_last = global_step.numpy() == num_steps_per_epoch * FLAGS.num_epochs
      def _torch_save(save_epoch=False):
        if rank == 0:
          state = {
                  'init_epoch': FLAGS.init_epoch,
                  'epoch': global_step.numpy() / num_steps_per_epoch,
                  'step': global_step.numpy(),
      ...
      epoch_save = False
      if FLAGS.save_interval_epochs \
        and (global_step.numpy() % int(num_steps_per_epoch * FLAGS.save_interval_epochs + 0.5) == 0 or is_last):
      ...
      epoch_valid = False
      # is_first = global_step.numpy() == start_step or global_step.numpy() == start_step + 1
      # TODO if reload model might eval after a few steps, because epoch save is before this step..
      # ((global_step.numpy() - epoch * num_steps_per_epoch) % int(num_steps_per_epoch * FLAGS.valid_interval_epochs + 0.5) == 0 or is_last or ((global_step.numpy() - epoch * num_steps_per_epoch) == 0 and (not is_first))) \
      
      if eval_dataset and (metric_eval_fn is not None) and (not FLAGS.train_only) and FLAGS.do_valid and FLAGS.metric_eval and  FLAGS.valid_interval_epochs > 0 and \
         ((global_step.numpy() % int(num_steps_per_epoch * FLAGS.valid_interval_epochs + 0.5) == 0 or is_last) \
          or (FLAGS.first_interval_epoch > 0 and global_step.numpy() == int(num_steps_per_epoch * FLAGS.first_interval_epoch + 0.5)) \
          or (FLAGS.eval_interval_steps and global_step.numpy() % FLAGS.eval_interval_steps == 0)):
        epoch_valid = True
        ...
        # TODO 如果没有在一轮末尾就最终结束了 需要再做最后的eval
        if not FLAGS.debug_loop:
      ...
      if FLAGS.debug_loop:
        if epoch_valid:
          ic(global_step.numpy(), epoch_valid, melt.epoch(), epoch)
  ...
  if trained_steps and (not is_last):
    if FLAGS.debug_loop:
      ic(global_step.numpy(), epoch_valid, melt.epoch(), epoch)
    else:
      is_last = True
      ...
      try:
        ...
        ic(e)
        logger.warning('try to do last eval but failed')
