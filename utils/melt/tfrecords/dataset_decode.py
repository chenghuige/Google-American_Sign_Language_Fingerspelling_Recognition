...
def inputs(files, 
  ...
  # d = threadpool.override_threadool(
  #   d,
  ...
  if cache and (not FLAGS.cache_after_map):
    logging.debug('Cache datase before map')
  ...
  if not batch_parse:
    ...
    # d = d.map(decode_fn, num_parallel_calls=16)
  ...
  if bucket_boundaries:
    ...
    logging.debug(batch_parse, dynamic_pad)
    if dynamic_pad:
  ...
  if FLAGS.copy_to_device:
    d = d.apply(tf.data.experimental.copy_to_device("/gpu:0"))
