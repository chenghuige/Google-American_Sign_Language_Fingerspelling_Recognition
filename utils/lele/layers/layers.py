...
class NonLinearSeqAttn(nn.Module):
  ...
  def __init__(self, input_size, hidden_size=128, activation='relu'):
    ...
    self.FFN = FeedForwardNetwork(input_size,
                                  hidden_size,
                                  1,
...
class MeanPooling(nn.Module):
  ...
  def forward(self, x, x_mask=None):
    ...
    input_mask_expanded = x_mask.unsqueeze(-1).expand(x.size()).float()
...
def dense_layer(inp, out, dropout=0., activation='ReLU'):
  ...
  return nn.Sequential(nn.Linear(inp, out), activate(), nn.Dropout(dropout))
...
# dropout on batch size dim, equal to tf.dropout with noise_shape=(None,1,1)
