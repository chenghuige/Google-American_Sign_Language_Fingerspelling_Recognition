...
def dice_coef(y_pred, y_true, smooth=1e-4, ignore_background=False):
  loss = None
...
def dice_loss(y_pred, y_true, smooth=1e-4, ignore_background=False):
  return 1 - dice_coef(
...
def dice_coef2(y_pred, y_true, smooth=1e-4, ignore_background=False):
  loss = None
  #ic(y_pred, y_true, y_pred.shape, y_true.shape)
  start = 0 if not ignore_background else 1
...
def dice_loss2(y_pred, y_true, smooth=1e-4, ignore_background=False):
  return 1 - dice_coef2(
      y_pred, y_true, smooth=smooth, ignore_background=ignore_background)
...
#   def __init__(self, smooth=1e-4, ignore_background=False, reduction='mean'):
#     self.smooth = smooth
...
class ContrastiveLoss(nn.Module):
  ...
  def __init__(self, margin=0.5, eps=1e-5):
  ...
  def forward(self, pred, label):
    ...
    # ic(distance, label, loss)
    return loss.sum()

# https://www.kaggle.com/code/rejpalcz/best-loss-function-for-f1-score-metric/comments
def f1_loss(y_pred, y_true):
  tp = (y_true * y_pred).float().sum(-1)
  tn = ((1 - y_true) * (1 - y_pred)).float().sum(-1)
  fp = ((1 - y_true) * y_pred).float().sum(-1)
  fn = (y_true * (1 - y_pred)).float().sum(-1)

  f1 = tp / (tp + 0.5 * fp + 0.5 * fn + 1e-4)
  f1= torch.nan_to_num(f1)
  return 1 - f1.mean()

def f2_loss(y_pred, y_true):
  tp = (y_true * y_pred).float().sum(-1)
  tn = ((1 - y_true) * (1 - y_pred)).float().sum(-1)
  fp = ((1 - y_true) * y_pred).float().sum(-1)
  fn = (y_true * (1 - y_pred)).float().sum(-1)

  f2 = tp / (tp + 0.2 * fp + 0.8 * fn + 1e-4)
  f2 = torch.nan_to_num(f2)
  return 1 - f2.mean()
