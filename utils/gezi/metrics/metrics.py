...
# for calc auc same result below
# auc = gezi.metrics.fast_auc(gts, preds, use_numba=False)  # super fast 5s
# auc = 1. - gezi.metrics.inverse_ratio(gts, preds) # similar as above but a bit slower 8s
# auc = sklearn.metrics.roc_auc_score(gts, preds) # slow 30s+
...
try:
  @njit
  def _auc_numba(actual, pred_ranks):
    n_pos = np.sum(actual)
    n_neg = len(actual) - n_pos
    return (np.sum(pred_ranks[actual == 1]) - n_pos *
            (n_pos + 1) / 2) / (n_pos * n_neg)
...
def fast_auc(actual, predicted, use_numba=True):
  # https://www.kaggle.com/c/riiid-test-answer-prediction/discussion/208031
  pred_ranks = rankdata(predicted)
  if use_numba:
    return _auc_numba(actual, pred_ranks)
