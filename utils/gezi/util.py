...
def get_singles(l):
  ...
  """
  return [x for x in l if not isinstance(x, Iterable)]
...
# pympy more flexible but high probablity of hang, especailly if you call it multiple times, so prefer to use pandaparallel
def parallel_run_list(func, inputs, num_workers=None, desc='run', verbose=None, leave=True, pymp=False):
  if verbose is None:
    if not pymp:
      verbose = 2
    else:
  ...
  if verbose > 1:
    timer = gezi.Timer(f'parallel_run_list:{desc}', True)
  ...
  if num_workers == 1:
    res = [func(x) for x in tqdm(inputs, desc=desc, leave=leave, disable=(not (verbose and rank == 0)))]
  else:
    if pymp:
      num_workers = min(num_workers, len(inputs))
      ...
      #     m[i] = [func(x) for x in tqdm(inputs_list[i], desc=desc, leave=leave, disable=(not (verbose and i==0 and rank==0)))]
          
      # res = []
      # for i in tqdm(range(num_workers), desc='merge', leave=False):
      #   res.extend(m[i])
      
      ar = pymp.shared.list([None] * num_workers)
      with pymp.Parallel(num_workers) as p:
        for i in p.range(num_workers):
          ar[i] = [func(x) for x in tqdm(inputs_list[i], desc=desc, leave=leave, disable=(not (verbose and i==0 and rank==0)))]
      ...
      # res = []
      # aprun = ParallelExecutor(n_jobs=num_workers)
  #    File "/root/miniconda3/lib/python3.8/site-packages/joblib/pool.py", line 156, in send
  #    CustomizablePickler(buffer, self._reducers).dump(obj)
  # AttributeError: Can't pickle local object 'init_dfs.<locals>.<lambda>'
      # results = Parallel(n_jobs=num_workers, backend="multiprocessing")(
      #     delayed(func)(inputs) for inputs in inputs_list
      # )
      # with Pool(num_workers) as p:
      #   results = p.map(func, inputs)
      # ic(results)
      # resutls = aprun(total=len(inputs))(delayed(func)(a) for a in inputs_list)
      # for result in tqdm(results, desc='merge', leave=False):
      #   res.extend(result)
    else:
      df = pd.DataFrame({'inputs': inputs})
      res = df['inputs'].parallel_apply(func)
...
def parallel_run_dict(func, inputs, num_workers=None, desc='run', verbose=1, leave=True):
  if verbose > 1:
    timer = gezi.Timer('parallel_run_dict')
  rank = gezi.get('RANK', 0)
  if inputs is None or not len(inputs):
    return []
  if len(inputs) == 1:
...
def parallel_run_loop(func, inputs, num_workers=None, desc='run', verbose=1, leave=True):
  if verbose > 1:
    timer = gezi.Timer('parallel_run_loop')
  rank = gezi.get('RANK', 0)
  if inputs is None or not len(inputs):
    return []
  if len(inputs) == 1:
  ...
  if not num_workers:
    num_workers = cpu_count()
  if num_workers == 1:
    for x in tqdm(inputs, desc=desc, leave=leave, disable=(not (verbose and rank == 0))):
      func(x)
  else:
    num_workers = min(num_workers, len(inputs))
    ...
    try:
      ...
      with pymp.Parallel(num_workers) as p:
        for i in p.range(num_workers):
          for x in tqdm(inputs_list[i], desc=desc, leave=leave, disable=not (verbose and i == 0 and rank == 0)):
            func(x)
    except Exception:
      with Pool(num_workers) as p:
        p.map(func, inputs)
...
def dump_flags(model_dir=None, flags_file='flags.pkl'):
  from absl.testing import flagsaver
  flag_values = flagsaver.save_flag_values()
  model_dir = model_dir or FLAGS.model_dir
  with open(f'{model_dir}/{flags_file}', 'wb') as fout:
...
def get_gpus(min_free_mem=None, max_used_mem=None, env_limit=False):
  ...
  if num_gpus is not None:
    return get_specific_gpus()
  
  try:
    gpu_stats = subprocess.check_output(
        ["nvidia-smi", "--format=csv", "--query-gpu=memory.used,memory.free"])
  except Exception as e:
    # TODO why might nvidia-smi fail?
    ic(e)
  ...
  if not names:
    ...
    if os.path.exists(header):
      ...
      if os.path.exists(header):
        ...
        names = open(file).readline().strip().split(sep)
        if not skip_rows:
          skip_rows = 1
...
def in_kaggle():
  return os.path.exists('/kaggle') and os.uname()[1] != 'gezi'
...
def load_weights(model, model_path, map_location=None, strict=True):
  ...
  if not FLAGS.torch:
    ...
    import lele
    lele.load_weights(model, model_path, map_location=map_location, strict=strict)
...
def save_model(model, model_path, fp16=True, epoch=None, step=None):
  if FLAGS.torch:
    import lele
    lele.save_model(model, model_path, fp16=fp16, epoch=epoch, step=step)
  else:
    import melt
...
def folds_metrics_summary(model_dir, folds='folds'):
  ...
  from gezi import summary
  sw = gezi.summary.SummaryWriter(log_dir)
  ...
  for fold in range(folds):
    ...
    total += df['insts'].values[-1]
    for col in cols:
      m[col] += df[col].values[-1] * df['insts'].values[-1]
      sw.log({
      ...
      sw.log({
          f'Metrics/mean/{col}': m[col] / total,
          'Metrics/step': fold,
          'step': fold
      })
...
def sort_dict_byval(m, ascending=False):
  return sorted(((v, k) for k, v in m.items()), reverse=not ascending)
sort_byval = sort_dict_byval
...
def set_fold(df, folds=5, group_key=None, stratify_key=None, seed=1024):
...
def format_float(x, precision):
  return format_float_str(x, precision)
...
def read_df(ifile, convert=False):
  if ifile.endswith('.csv'):
    ...
    try:
      ...
      df = pd.read_csv(ifile.replace('.fea', '.csv'))
      if convert:
        df.reset_index().to_feather(ifile)
  return df

def write_df(df, ofile):
...
def trunct_lens(input_lens, max_len, method='ratio'):
  ...
  avg_len2 = int(left_len / len(long_lens))
  def get_len(x):
    if x <= avg_len:
      return x
    else:
...
def topk(x, k):
  if len(x) > k:
    indexes = np.argpartition(x, -k)[-k:]
    values = x[indexes]
    indexes = indexes[np.argsort(-values)]
  else:
...
def slow_disk():
  return os.path.exists('../working/wangpan')
...
def finish():
  finish_wandb()
  logging.info(f'finish {FLAGS.model_dir}')
