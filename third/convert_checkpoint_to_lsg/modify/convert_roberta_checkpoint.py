...
def update_positions(model, max_pos):
    position_embeddings_weights = model.roberta.embeddings.position_embeddings.weight.clone()
    ...
    model.roberta.embeddings.position_ids = torch.arange(max_pos + 2, device=model.roberta.embeddings.position_ids.device).unsqueeze(0)
    model.roberta.embeddings.position_embeddings.weight.data = new_position_embeddings_weights
    return model
...
def update_global(model, bos, mask, stride, keep_first=False):
    u = model.roberta.embeddings.word_embeddings.weight.clone()
    positions = model.roberta.embeddings.position_embeddings.weight.clone()[2:]
    positions = order_positions(positions, stride)
    positions[0] += u[bos]
    positions[1:] += u[mask].unsqueeze(0)

    if keep_first:
        model.roberta.embeddings.global_embeddings.weight.data[1:] = positions[1:]
    else:
        model.roberta.embeddings.global_embeddings.weight.data = positions
    return model
...
def update_global_randomly(model, bos, stride, keep_first=False):
    ...
    if keep_first:
        model.roberta.embeddings.global_embeddings.weight.data[1:] = (w + positions)[1:]
    else:
        model.roberta.embeddings.global_embeddings.weight.data = w + positions
    return model
