...
class Superviser():
  ...
  def run(self):
    global port
    if not ('--seed=' in self.command or '--seed ' in self.command):
      seed = random.randint(0, 100000)
      self.command += f' --seed={seed}'
    self.command += ' --distributed'
    if '--port=' in self.command or '--port ' in self.command:
      command = self.command.replace('=', ' ')
      port = command.split()[command.split().index('--port') + 1]
    gpus = gezi.get_gpus()
